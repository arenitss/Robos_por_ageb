...
# Reemplazamos con 0 los NA's de las columnas:
...
ageb_censo_delitos_wifi_t1 <- ageb_censo_delitos_wifi %>%
  mutate(tasa_rob_veh = robo_vehículos/pobtot*1000) %>% relocate(tasa_rob_veh)

ggplot(ageb_censo_delitos_wifi_t1, aes(y = tasa_rob_veh)) +
...
ageb_censo_delitos_wifi_t2 <- ageb_censo_delitos_wifi %>%
  mutate(robo_vehicular = ifelse(robo_vehículos>0, 1, 0)) %>% relocate(robo_vehicular)
ageb_censo_delitos_wifi_t2
